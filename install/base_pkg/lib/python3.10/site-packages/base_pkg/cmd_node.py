...
# USING_TTL = '/dev/input/by-id/usb-ITE_Tech._Inc._ITE_Device_8910_-event-if00'
USING_TTL = RED_TTL
...
class CmdNode(Node):
    ...
    def joy_callback(self, msg):
        ...
        if (dt > 0.05):
            ...
            self.twist.linear.x = map_value(msg.axes[1], 1.0, -1.0,
                           -MAX_VELOCITY, MAX_VELOCITY) * speedFactor
            # verticle of left joystick
            self.twist.linear.y = map_value(msg.axes[0], -1.0,  1.0,
                           -MAX_VELOCITY, MAX_VELOCITY) * speedFactor
